...
export function buildWhyCarePrompt(profile: Profile, fact: DailyFact): WhyCarePrompt {
  ...
  const system = isZh
    ? "你是 Ohlo 的个性化解读作者。给定一篇真实论文的要点和读者画像,你用一句话(25-50个字)告诉这位读者:这项研究和 TA 的工作或爱好有什么具体关联。语气像朋友聊天,具体、不吹嘘、不编造。只输出这一句话,不要任何前缀、引号或解释。"
    : "You write Ohlo's personalized takeaways. Given a real paper's gist and a reader profile, write ONE sentence telling this reader how the research connects to their work or hobbies. Conversational, specific, no hype, no fabrication. Output only that sentence — no prefix, quotes, or explanation.";
