...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await client
      ...
      .select(
        "paper_id, created_at, papers!inner(id, arxiv_id_base, arxiv_id_version, title, hook_summary_en, hook_summary_zh, plain_summary_en, plain_summary_zh, human_category, tags, primary_category, categories, published_at, abs_url, pdf_url)",
      )
      .eq("user_id", user.id)
    ...
    const items = ((data as Array<Record<string, unknown>> | null) ?? []).map((row) => {
      ...
      return {
        ...
        hookSummaryZh: String(paper.hook_summary_zh ?? ""),
        personalizedHook: String(paper.hook_summary_en ?? ""),
        plainSummary: String(paper.plain_summary_en ?? ""),
        plainSummaryZh: String(paper.plain_summary_zh ?? ""),
        humanCategory: String(paper.human_category ?? "AI & Robots"),
